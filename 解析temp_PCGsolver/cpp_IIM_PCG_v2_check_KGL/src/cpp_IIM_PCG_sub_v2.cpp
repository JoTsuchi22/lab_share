...
void Make_coupled_K_EL(int El_No_loc, int El_No_glo, double *coupled_K_EL, information *info)
{
	...
	for (j = 0; j < info->No_knot[patch * info->DIMENSION + 0]; j++)
	{
		temp_Position_Knots_xi[j] = info->Position_Knots[info->Total_Knot_to_patch_dim[patch * info->DIMENSION + 0] + j];
	}
	for (j = 0; j < info->No_knot[patch * info->DIMENSION + 1]; j++)
	{
		temp_Position_Knots_eta[j] = info->Position_Knots[info->Total_Knot_to_patch_dim[patch * info->DIMENSION + 1] + j];
	}
	...
	if (El_No_loc == number)
	{
		...
		for (i = 0; i < div * div; i++)
		{
			...
			int aa = i / div;
			int bb = i % div;
			loc_para[0] = ((double)(aa) / ((double)div - 1.0)) * 2 - 1;
			loc_para[1] = ((double)(bb) / ((double)div - 1.0)) * 2 - 1;

			Make_BG_Matrix(El_No_loc, B, loc_para, info);
			...
			for (j = 0; j < info->DIMENSION; j++)
			{
				...
				{
					a_2x2[j][k] = 0.0;
					for (l = 0; l < info->No_Control_point_ON_ELEMENT[info->Element_patch[El_No_loc]]; l++)
					{
						double dsf = dShape_func(l, k, loc_para, El_No_loc, info);
						a_2x2[j][k] += dsf * info->Node_Coordinate[info->Controlpoint_of_Element[El_No_loc * MAX_NO_CP_ON_ELEMENT + l] * (info->DIMENSION + 1) + j];
					}
				}
			}
			J = InverseMatrix_2x2(a_2x2);
			...
			double data_result_shape[MAX_DIMENSION] = {0.0};
			for (j = 0; j < info->No_Control_point_ON_ELEMENT[info->Element_patch[El_No_loc]]; j++)
			{
				double R = Shape_func(j, loc_para, El_No_loc, info);

				for (k = 0; k < info->DIMENSION; k++)
				{
					data_result_shape[k] += R * info->Node_Coordinate[info->Controlpoint_of_Element[El_No_loc * MAX_NO_CP_ON_ELEMENT + j] * (info->DIMENSION + 1) + k];
				}
			}

			Calc_xi_eta(data_result_shape[0], data_result_shape[1],
						temp_Position_Knots_xi, temp_Position_Knots_eta,
						info->No_Control_point[patch * info->DIMENSION + 0], info->No_Control_point[patch * info->DIMENSION + 1],
						info->Order[patch * info->DIMENSION + 0], info->Order[patch * info->DIMENSION + 1],
						&para[0], &para[1], info);
