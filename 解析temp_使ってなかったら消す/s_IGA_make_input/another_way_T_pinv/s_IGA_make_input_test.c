...
void KR_calc_Tinv_1D(int tm, int removal_parameter_axis)
{
    ...
    double B[l][l];
    double B_inv[l][l];
    // double A_pinv_T[l + vec_length2[tm][removal_parameter_axis]][l];
    double A_pinv[l][l + vec_length2[tm][removal_parameter_axis]];
    ...
    for (i = 0; i < l; i++)
    {
        for (j = 0; j < l + vec_length2[tm][removal_parameter_axis]; j++)
        {
            ...
            temp_x2[i] += A_pinv[i][j] * temp_x1[j];
            temp_y2[i] += A_pinv[i][j] * temp_y1[j];
            temp_w2[i] += A_pinv[i][j] * temp_w1[j];
        }
    }
